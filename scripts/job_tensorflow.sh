...
for i in {1..1}; do
  RUN_DIR="/home1/08486/mmiranda/tensorflow/scripts/lenet-100g-bs${BATCH_SIZE}-ep${EPOCHS}-${DATE}"
  #remora strace -cf -o strace_tensor_local_e3_t2.txt ./train.sh -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -v -d "$DATASET" -r $RUN_DIR 
  #remora strace -cf -o strace_loadllll.txt ./train.sh -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -v -d "$DATASET" -r $RUN_DIR 
