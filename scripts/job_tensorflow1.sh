...
for i in {1..1}; do
  ...
  #./train_global.sh -j 1 -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -d "$DATASET" -r $RUN_DIR > aaaa2.txt
  #./train_global.sh -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -v -d "$DATASET" -r $RUN_DIR -j 1
  #strace -cf -o strace_tensor_dist_e3_c5_b256.5_n1.txt ./train_global.sh -j 1 -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -v -d "$DATASET" -r $RUN_DIR
  ./train_global.sh -j 1 -o -m lenet -b $BATCH_SIZE -e $EPOCHS -g 4 -i autotune -l -v -d "$DATASET" -r $RUN_DIR
  sleep 10
