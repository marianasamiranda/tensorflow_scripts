...
OUTPUT=$(squeue -u mmiranda  | awk 'NR > 1 {print $8}'| head -n 1)

echo "$OUTPUT"

NODE1=$(echo $OUTPUT | sed 's/\(c.*-\)\[\(.*\)[,-]\(.*\)[,-]\(.*\)\]/\1\2:2222/')
NODE2=$(echo $OUTPUT | sed 's/\(c.*-\)\[\(.*\)[,-]\(.*\)[,-]\(.*\)\]/\1\3:2222/')
NODE3=$(echo $OUTPUT | sed 's/\(c.*-\)\[\(.*\)[,-]\(.*\)[,-]\(.*\)\]/\1\4:2222/')
...
#WORKER_HOSTS="c197-021:2222"
WORKER_HOSTS="${NODE1},${NODE2},${NODE3}"
#WORKER_HOSTS="c196-091,c197-042"
